...
var installAchievementSystem_d38bb038_c4fa_43be_927c_168680046baa = function (core, plugin) {
	...
	var LEVELS = {
		bronze: { name: "青铜", color: "#96602f", glow: "rgba(201,139,82,.35)" },
		platinum: { name: "白金", color: "#857143", glow: "rgba(238,231,210,.34)" },
		diamond: { name: "钻石", color: "#2b7895", glow: "rgba(112,225,245,.34)" }
	};
	var LEVEL_ALIASES = {
	...
	var openIconViewer = function (definition, trigger) {
		...
		var viewerHeading = document.createElement("div");
		viewerHeading.className = "achievement-icon-viewer-heading";
		var viewerTitle = document.createElement("strong");
		viewerTitle.textContent = "荣誉印记";
		viewerHeading.appendChild(viewerTitle);
		viewerHeading.appendChild(close);
		card.appendChild(viewerHeading);
		card.appendChild(largeIcon);
		card.appendChild(caption);
		iconViewer.appendChild(card);
		root.appendChild(iconViewer);
		theme.decorate(card, { radius: 24, ornate: true, crest: true, fill: "blue" });
	...
	var showNextToast = function () {
		...
		theme.decorate(toast, { radius: 17, fill: "gold", crest: true });
		playUnlockSound();
	...
	var renderPreview = function () {
		...
		theme.releaseTree(statsRoot);
		statsRoot.innerHTML = "";

		var appendStat = function (title, unlocked, total, levelKey) {
			var selected = activeLevelFilter === levelKey;
			var filterKey = levelKey || "all";
			var card = document.createElement("button");
			card.type = "button";
			card.className = "achievement-stat" + (levelKey ? " level-" + levelKey : " achievement-stat-total");
			card.dataset.levelFilter = filterKey;
			card.setAttribute("aria-pressed", String(selected));
			card.setAttribute("aria-controls", "achievement-filtered-list");
			card.setAttribute("aria-label", title + "，已解锁 " + unlocked + " / " + total);
			card.addEventListener("click", function () {
				if (activeLevelFilter === levelKey) return;
				activeLevelFilter = levelKey;
				renderPreview();
				listRoot.scrollTop = 0;
				// 重建统计后将焦点交还当前筛选按钮，支持连续使用键盘切换。
				statsRoot.querySelector('[data-level-filter="' + filterKey + '"]').focus({ preventScroll: true });
			});
			if (levelKey) card.style.setProperty("--achievement-level-color", LEVELS[levelKey].color);
			var label = document.createElement("span");
			label.className = "achievement-stat-label";
			...
			track.setAttribute("aria-valuenow", unlocked);
			track.appendChild(progress);
			card.appendChild(track);
			statsRoot.appendChild(card);
			theme.decorate(card, { radius: 12, fill: selected ? "gold" : "pearl",
				selected: selected, interactive: true, bleed: 20 });
		};
		...
		theme.releaseTree(listRoot);
		listRoot.innerHTML = "";
		getEntries().forEach(function (entry) {
			if (activeLevelFilter && entry.level !== activeLevelFilter) return;
			...
			theme.decorate(row, { radius: 15, fill: entry.unlocked ? "gold" : "pearl",
				selected: entry.unlocked, interactive: true });
		});
	};
	...
	var closePreview = function () {
		...
		if (previewTrigger && previewTrigger.isConnected) previewTrigger.focus();
		previewTrigger = null;
		return true;
	};
	...
	var openPreview = function () {
		...
		statsRoot.setAttribute("role", "group");
		statsRoot.setAttribute("aria-label", "成就等级筛选");
		listRoot = document.createElement("div");
		listRoot.className = "achievement-list";
		listRoot.id = "achievement-filtered-list";
	...
	var installStyles = function () {
		...
		style.href = "project/achievement.css?v=" + main.version;

...
本项目把背包武器属性集中保存在 `project/weapons.js`，`project/items.js` 只保存地图物品本身和 `backpackWeaponId` 映射。为了避免为了使用编辑器而复制一份武器属性，物品图块属性表使用了虚拟合并：

1. 在素材区选择带有 `backpackWeaponId` 的物品。
2. 打开“图块属性”，表格会在普通物品属性后显示 `weapon`（背包武器属性）分组。
3. 修改该分组中的形状、稀有度、伤害上下限、命中率、攻击间隔、奥义获取、类型或联动规则。
4. 编辑器保存时会把普通物品字段写回 `project/items.js`，把 `weapon` 分组字段写回 `project/weapons.js`。

因此游戏运行时和编辑器始终读取同一份武器定义。修改 `backpackWeaponId` 后，需要重新选中该物品，表格才会载入新关联的武器数据。

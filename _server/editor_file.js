...
editor_file = function (editor, callback) {
    ...
    editor.file.editItem = function (id, actionList, callback) {
        ...
        */
        ...
        if (isset(actionList) && actionList.length > 0) {
            var itemActions = [];
            var weaponActions = [];
            var weaponPrefix = "['weapon']";
            var item = items_296f5d02_12fd_4166_a7c1_b5e830c9ee3a[id] || {};
            var weaponId = item.backpackWeaponId;
            actionList.forEach(function (value) {
                value = value.slice();
                if (value[1].indexOf(weaponPrefix) === 0) {
                    value[1] = value[1].substring(weaponPrefix.length);
                    weaponActions.push(value);
                } else {
                    value[1] = "['" + id + "']" + value[1];
                    itemActions.push(value);
                }
            });

            if (weaponActions.length > 0 && !weaponId) {
                callback(['该物品没有配置 backpackWeaponId，无法保存武器属性']);
                return;
            }
            if (weaponActions.length > 0 && !weaponDefinitions_9f2e6f5b_4b2c_4f8c_9a3d_7e1b6c0d5a44[weaponId]) {
                weaponDefinitions_9f2e6f5b_4b2c_4f8c_9a3d_7e1b6c0d5a44[weaponId] = {};
            }
            weaponActions.forEach(function (value) {
                value[1] = "['" + weaponId + "']" + value[1];
            });

            var pending = (itemActions.length > 0 ? 1 : 0) + (weaponActions.length > 0 ? 1 : 0);
            var errors = [];
            var done = function (err) {
                if (err) errors.push(err);
                pending--;
                if (pending === 0) callback([errors.length ? errors.join('\n') : null]);
            };
            if (itemActions.length > 0) saveSetting('items', itemActions, done);
            if (weaponActions.length > 0) saveSetting('weapons', weaponActions, done);
        } else {
            callback([
                (function () {
                    ...
                    var item = items_296f5d02_12fd_4166_a7c1_b5e830c9ee3a[id] || {};
                    if (item.backpackWeaponId) {
                        locObj.weapon = core.clone(
                            weaponDefinitions_9f2e6f5b_4b2c_4f8c_9a3d_7e1b6c0d5a44[item.backpackWeaponId] || {}
                        );
                    }
                    return locObj;
                })(),
    ...
    var saveSetting = function (file, actionList, callback) {
        ...
        if (file == 'weapons') {
            actionList.forEach(function (value) {
                _update("weaponDefinitions_9f2e6f5b_4b2c_4f8c_9a3d_7e1b6c0d5a44", value);
            });
            var datastr = [
                '/**',
                ' * 背包乱斗武器的唯一配置源。',
            ...
            datastr += JSON.stringify(weaponDefinitions_9f2e6f5b_4b2c_4f8c_9a3d_7e1b6c0d5a44, null, '\t');
            fs.writeFile('project/weapons.js', encode(datastr), 'base64', function (err, data) {
                callback(err);
            });
            return;
        }

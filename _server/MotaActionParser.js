...
ActionParser.prototype.parseAction = function() {
  ...
  switch (data.type) {
    ...
    x_str.join(','),y_str.join(','),data.floorId||'',data.remove||false, data.time, data.destruct||false, data.async||false, this.next]);
      break;
    case "setBlock": // 设置图块
      data.loc=data.loc||[];
      if (!(data.loc[0] instanceof Array))
    ...
    data.number||0,x_str.join(','),y_str.join(','),data.floorId||'', data.destruct||false, data.time, data.async||false, this.next]);
      break;
    ...
    case "generateMove": // 生成并移动图块事件
      ...
      this.next = MotaActionBlocks['generateMove_s'].xmlText([
          data.loc[0],
          data.loc[1],
          data.id,          // 图块ID（作为EvalString处理）
          data.time,        // 动画时间
          data.keep,        // 不消失
          data.fadeInTime,  // 淡入时间
          data.zIndex,      // Z轴层级
          data.async,       // 不等待执行完毕
    ...
    case "confirm": // 显示确认框
      this.next = MotaActionFunctions.xmlText('confirm_s', [
        this.EvalString_Multi(data.text), data.timeout||0, data.norecord||false,  // 添加norecord字段
        data["default"], this.insertActionList(data["yes"]),
        this.insertActionList(data["no"]), this.next], 
        /* isShadow */false, /*comment*/ null, /*collapsed*/ data._collapsed, /*disabled*/ data._disabled);
      break;
    case "switch": // 多重条件分歧
    ...
    case "choices": // 提供选项
      ...
      for(var ii=data.choices.length-1,choice;choice=data.choices[ii];ii--) {
        ...
        text_choices=MotaActionFunctions.xmlText('choicesContext', [
          choice.text,choice.icon,choice.color,'rgba('+choice.color+')',
          choice.need||'',choice.condition||'',choice.norecord||false,  // 添加norecord字段
          this.insertActionList(choice.action),text_choices],
      ...
      this.next = MotaActionFunctions.xmlText('choices_s', [
        info[3],info[0],info[1],data.timeout||0,data.width,data.norecord||false,  // 添加norecord字段
        text_choices,this.next], /* isShadow */false, /*comment*/ null, /*collapsed*/ data._collapsed, /*disabled*/ data._disabled);
      break;
    case "for": // 循环遍历
